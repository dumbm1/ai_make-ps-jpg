...
makeSepBtn.addEventListener("click", (e) => {
 csInterface.evalScript(jsx_helpForSep.toString() + ';jsx_helpForSep();', function (result) {
  const infoStringSpan = document.querySelector('.footer__info-string-span');

  if (!result) {
   infoStringSpan.innerText += 'sep error: ' + result;
   return;
  }
  if (result.match('Uncaught JavaScript exception')) {
   infoStringSpan.innerText += 'sep (' + formatTime(new Date()) + '): Error\n';
   return;
  }
  let res = Math.round(result);
  res = 'sep (' + formatTime(new Date()) + ') \n';
